...
1. 主进程启动 ffmpeg 进程，将 rtsp 流转换为 flv 流。
2. 主进程启动 WebSocket 服务器，将 flv 流推送到 WebSocket 客户端。
3. 渲染进程启动 WebSocket 客户端，从 WebSocket 服务器接收 flv 流。
4. 渲染进程使用 flv.js 播放 flv 流。
